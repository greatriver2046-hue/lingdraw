...
| 功能大类          | 具体功能                                      |
| ------------- | ----------------------------------------- |
| **AI 图片生成**   | 文本生成图片、参考图生成、多图参考生成                       |
| **AI 图片编辑**   | 局部重绘、擦除修复、背景抠图、超分辨率放大、去水印、姿态编辑            |
| **AI 视频生成**   | 文本/图片转视频、支持首帧/尾帧参考、多图参考                   |
| **AI 对话**     | 多模型 LLM 对话（GPT / Claude / GLM / 豆包 / 通义等） |
| **AI 写作**     | 资料评估、检索计划、事实提炼、大纲生成、风格迁移改写                |
| **风格分析**      | 作者风格提取、风格迁移                               |
| **支付系统**      | 微信支付、支付宝（套餐购买）                            |
| **会员体系**      | 多套餐（周 / 月 / 季度 / 年度）、积分系统、周期重置            |
| **多租户**       | SaaS 实例隔离、租户独立配置                          |
| **WebSocket** | 实时生成进度推送                                  |
| **OSS 存储**    | 阿里云 OSS 集成                                |
| **短信服务**      | 注册 / 登录 / 找回密码短信                          |
