    ...
    <div class="login-content">
      <div class="login-left">
        <div class="login-bg-content">
          <h1 class="welcome-title">AI SaaS Platform</h1>
          <p class="welcome-desc">一站式 AI 内容生成与管理平台</p>
...
import { User, Lock, Monitor, DataLine, UserFilled } from '@element-plus/icons-vue'
...
const handleLogin = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      loading.value = true
      try {
        if (await userStore.login(form.username, form.password)) {
...
onMounted(() => {
  const ssoToken = route.query.sso_token || route.query.token
  if (typeof ssoToken === 'string' && ssoToken.trim()) {
    ...
    if (typeof rawUser === 'string' && rawUser.trim()) {
      try {
        const info = JSON.parse(rawUser)
        localStorage.setItem('tenant_user', JSON.stringify(info))
        userStore.user = info
      } catch {
